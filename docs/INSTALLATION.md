...
- **Providers configured** in `opencode.json`:
  - `openrouter` provider with the two harness models:
    - `openai/gpt-5.6-luna`
...
No merge needed if these names don't already exist; if you have an existing `agents/` dir, copy only the `luna/`, `v4/`, `dual/` subdirectories.
...
2. **Web on V4 researchers**: `opencode debug agent v4/researcher` → `webfetch → allow`, `websearch → allow`.
3. **Models**: `luna/*` → `openrouter/openai/gpt-5.6-luna`; `v4/*`, `dual/orchestrator`, `dual/v4-*` → `openrouter/deepseek/deepseek-v4-flash-0731`.
4. **Delegation allowlists**: `debug agent` shows the `task` rules; confirm the catch-all `deny` precedes specific `allow`s (last-match-wins).
5. **subagent_depth**: leave at the default `1` (or set `"subagent_depth": 1` explicitly in `opencode.json`). This keeps agent trees one level deep — no recursive spawning.
...
Successful behavior (verified against 1.18.18):
...
Press **Tab** to cycle primary agents: `luna/build` → `v4/build` → `dual/orchestrator` → built-in `build`/`plan`. Pick the mode you want and start typing. Invoke subagents directly with `@luna/reviewer`, `@v4/researcher`, etc.

## 7. Uninstall

```bash
rm -rf ~/.config/opencode/agents/luna ~/.config/opencode/agents/v4 ~/.config/opencode/agents/dual
```
